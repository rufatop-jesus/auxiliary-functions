...
function o = particleOverlapNonSpherical(part,PB,rI,cd,xCd,yCd,zCd,alphaCd,betaCd,gammaCd,x,y,z,alpha,beta,gamma)
    o = false;
   
    for i = 1 : size(cd,1)
        if cd(i,1) ~= part
            if overlap([x; y; z],alpha,beta,gamma,rI, ...
                       [xCd(i,1); yCd(i,1); zCd(i,1)], alphaCd(i,1),betaCd(i,1),gammaCd(i,1),PB)
...
function o = overlap(OA,alphaA,betaA,gammaA,rA, OB,alphaB,betaB,gammaB,PBB)
    ...
    for i = 1 : size(rB,2) 
        % posAzi(1,i)
        % posEle(1,i)
        if  0 > rB(1,i) - rA(posEle(1,i)+91, posAzi(1,i)+181)
            o = true;
            return
        end
    end
